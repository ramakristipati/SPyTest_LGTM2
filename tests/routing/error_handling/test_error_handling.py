...
def config_ip_topology(config='yes'):
    ...
    thread_info.append(utils.ExecAllFunc(ipapi.config_ip_addr_interface, vars.D2, vars.D2D1P1, eh_data.ipv4_nbr,
                                         eh_data.ipv4_mask_len, family=eh_data.af_ipv4, config=config))
    thread_call(thread_info)

    thread_info = list()
    thread_info.append(utils.ExecAllFunc(ipapi.config_ip_addr_interface, vars.D1, vars.D1D2P1, eh_data.ipv6_addr,
                                         eh_data.ipv6_mask_len, family=eh_data.af_ipv6, config=config))
    thread_info.append(utils.ExecAllFunc(ipapi.config_ip_addr_interface, vars.D2, vars.D2D1P1, eh_data.ipv6_nbr,
                                         eh_data.ipv6_mask_len, family=eh_data.af_ipv6, config=config))
    thread_call(thread_info)
...
def config_nbr2(action):
    if action == "add":
        arpapi.add_static_arp(vars.D1, eh_data.ipv4_nbr2, eh_data.ipv4_nbr2_mac, interface=vars.D1D2P1)
        arpapi.config_static_ndp(vars.D1, eh_data.ipv6_nbr2, eh_data.ipv6_nbr2_mac, vars.D1D2P1, operation="add")
    elif action == "delete":
        arpapi.delete_static_arp(vars.D1, eh_data.ipv4_nbr2, interface=vars.D1D2P1, mac=eh_data.ipv4_nbr2_mac)
        arpapi.config_static_ndp(vars.D1, eh_data.ipv6_nbr2, eh_data.ipv6_nbr2_mac, vars.D1D2P1, operation="del")


def config_nbr3(action):
    if action == "add":
        arpapi.add_static_arp(vars.D1, eh_data.ipv4_nbr3, eh_data.ipv4_nbr3_mac, interface=vars.D1D2P1)
        arpapi.config_static_ndp(vars.D1, eh_data.ipv6_nbr3, eh_data.ipv6_nbr3_mac, vars.D1D2P1, operation="add")
    elif action == "delete":
        arpapi.delete_static_arp(vars.D1, eh_data.ipv4_nbr3, interface=vars.D1D2P1, mac=eh_data.ipv4_nbr3_mac)
        arpapi.config_static_ndp(vars.D1, eh_data.ipv6_nbr3, eh_data.ipv6_nbr3_mac, vars.D1D2P1, operation="del")
...
def test_ft_eh_nt_notify():
    ...
    entry1 = {'nexthop': eh_data.ipv4_nbr2, 'interface': vars.D1D2P1}
    entry2 = {'nexthop': eh_data.ipv6_nbr2, 'interface': vars.D1D2P1}
    entry3 = {'nexthop': eh_data.ipv4_nbr3, 'interface': vars.D1D2P1}
    ...
    entry1 = {'nexthop': eh_data.ipv4_nbr2, 'interface': vars.D1D2P1, 'failure': eh_data.swss_rc_not_found,
              'operation': "remove"}
    entry2 = {'nexthop': eh_data.ipv6_nbr2, 'interface': vars.D1D2P1, 'failure': eh_data.swss_rc_not_found,
              'operation': "remove"}
    entry3 = {'nexthop': eh_data.ipv4_nbr3, 'interface': vars.D1D2P1, 'failure': eh_data.swss_rc_not_found,
              'operation': "remove"}
    entry4 = {'nexthop': eh_data.ipv6_nbr3, 'interface': vars.D1D2P1, 'failure': eh_data.swss_rc_not_found,
              'operation': "remove"}
...
def test_ft_eh_bgp_route_notify():
    ...
    log_msg1 = len(list_filter_and_select(log_out, filter_list=[eh_data.log_str_swss, eh_data.loopback4_1, vars.D1D2P1,
                                                                eh_data.swss_rc_exists]))
    log_msg2 = len(list_filter_and_select(log_out, filter_list=[eh_data.log_str_swss, eh_data.loopback6_1, vars.D1D2P1,
                                                                eh_data.swss_rc_exists]))
...
def test_ft_eh_rt_bgp_plus_static_routes():
    ...
    entry1 = {'route': eh_data.loopback4_1, 'subnet': '32', 'interface': vars.D1D2P1}
...
def test_ft_eh_redis_cli():
    ...
    if not ehapi.verify_error_db_redis(vars.D1, eh_data.error_neigh_table, nhp=eh_data.ipv4_nbr2, ifname=vars.D1D2P1,
        ...
        result = False
    if not ehapi.verify_error_db_redis(vars.D1, eh_data.error_neigh_table, nhp=eh_data.ipv6_nbr2, ifname=vars.D1D2P1,
                                       rc=eh_data.swss_rc_not_found, opcode='remove'):
        st.error("REDIS: entry_found_not_in_error_db for {} ".format(eh_data.error_neigh_table))
        result = False
    ...
    if not ehapi.verify_error_db_redis(vars.D1, eh_data.error_route_table, mask=eh_data.ipv4_mask_len,
                                       route=eh_data.ipv4_route, nhp=eh_data.ipv4_nbr, ifname=vars.D1D2P1,
    ...
    if not ehapi.verify_error_db_redis(vars.D1, eh_data.error_route_table, mask=eh_data.ipv6_mask_len,
                                       route=eh_data.ipv6_route_sf, nhp=eh_data.ipv6_nbr, ifname=vars.D1D2P1,
                                       rc=eh_data.swss_rc_exists, opcode="create"):
        st.error("REDIS: entry_found_not_in_error_db for {} ".format(eh_data.error_route_table))
        result = False
...
def test_ft_eh_nt_notify_table_full():
    ...
    entry1 = {'nexthop': eh_data.ipv4_nbr2, 'interface': vars.D1D2P1, 'failure': eh_data.swss_rc_table_full,
              'operation': "create"}
    entry2 = {'nexthop': eh_data.ipv6_nbr2, 'interface': vars.D1D2P1, 'failure': eh_data.swss_rc_table_full,
              'operation': "create"}
